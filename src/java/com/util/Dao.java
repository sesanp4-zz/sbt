...
//            //multithread needs to be used for sending to thread
      ...
      public TransferLog getTransferLog(String accountNumber){
      
          com.entities.TransferLog transferLog=null;
          String today = LocalDate.now().toString();
      try{
          System.out.println("=========== Getting wallet transfer log Details started ========");
           session= HibernateUtil.getSessionFactory().openSession();
          session.beginTransaction();
          query =  session.createQuery("from TransferLog  transferLog where transferLog.accountNumber=:accountNumber and transferLog.dateCreated=:dateCreated");
          query.setParameter("accountNumber", accountNumber);
          query.setParameter("dateCreated", today);
          transferLog =(com.entities.TransferLog) query.uniqueResult();
          session.getTransaction().commit();
          System.out.println("== the trasfer object===="+gson.toJson(transferLog));
           System.out.println("=========== Done fetch Transaction Details =========");
           return transferLog;
      }catch(Exception e){
           System.out.println("=========== An Error has Occured =========");
