...
//        }
    ...
    public String nvalidateOTP(ValidateOTPRequest request) throws IOException{
       ...
       // set the parameters to be sent to the api
          ...
          String msg = EntityUtils.toString(response.getEntity());
          obj = new JsonParser().parse(msg).getAsJsonObject();
          System.out.println("response===="+obj.toString()+"...."+request.getTransaction().getLinkingreference());
          dao.nupdateTransactionEvent(request.getTransaction().getLinkingreference(),obj.get("code").getAsString(),obj.get("message").getAsString(),obj.get("transaction").getAsJsonObject().get("reference").getAsString());
          return msg;   
    } 
